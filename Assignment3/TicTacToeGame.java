...
public class TicTacToeGame {
    ...
    *  the number of cells that must be aligned to win.
	...
	public TicTacToeGame(int lines, int columns, int sizeWin){
		...
		if(lines==columns){
			transformations = new Transformation[]{Transformation.ID, Transformation.ROT, Transformation.ROT, Transformation.ROT, Transformation.HSYM ,Transformation.ROT, Transformation.ROT, Transformation.ROT};
		}
		else{
			transformations = new Transformation[]{Transformation.ID, Transformation.HSYM,Transformation.VSYM, Transformation.HSYM};
		}
	}
    ...
    *  String representation of the game
    ...
    public boolean hasNext(){
		...
		if (counter>=transformations.length){
			return false;
		}
		else{
			return true;
    ...
    public void next(){
    	...
    	// YOUR CODE HERE
		Transformation next = transformations[counter];
		switch(next){
			case ROT:
				counter+=1;
  	...
  	*/
  	public boolean equalsWithSymmetry(TicTacToeGame other){
		...
		while(hasNext()){
			...
			boolean flag = true;
			for(int i = 0; i<lines*columns;i++){
				if (other.valueAt(i)!=board[transformedBoard[i]]){
					flag = false;
				}
			}
